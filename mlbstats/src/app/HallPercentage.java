...
/*
 * Distribution of percentage of HOF players from 10-year players,
 * bin'd by last year of eligibility (5-year buckets), and by position
 */
public class HallPercentage {
  ...
  private static class Player {
    public int yearLast() { return _pWW.last().yearID(); }
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Master m : _master.all()) {
      ...
      if (yrs < 10 || lastYear > 2006) { continue; }
      Player p = new Player(m);
      total.add(p);
      ArrayList<Player> list = byYear.get(p.yearLast());
      if (list == null) { byYear.put(p.yearLast(), list = new ArrayList<>()); }
      list.add(p);
...
//      print(e.getKey().toString(), e.getValue());
      if (e.getKey() % 5 == 1) { print(e.getKey() - 4 + "-" + e.getKey(), year5); year5.clear(); } 
    }
